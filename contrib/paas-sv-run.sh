...
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! morphened has quit unexpectedly, checking for core dump and then starting a new instance..
  sleep 30
  ...
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    ...
    echo NOTIFYALERT! morphened stacktrace from coredump:
