...
Builds morphened to be a consensus-only low memory node. Data and fields not
...
### BUILD_MORPHENE_TESTNET=[OFF/ON]

Builds morphene for use in a private testnet. Also required for building unit tests.
...
We ship a Dockerfile.  This builds both common node type binaries.
    ...
    cd morphene
    docker build -t morphene/morphene morphene
## Building on Ubuntu 16.04

For Ubuntu 16.04 users, after installing the right packages with `apt` Morphene
...
Morphene requires Boost 1.58 (as in Ubuntu 16.04) and works with versions up to 1.60 (including).
So building Morphene on Ubuntu 14.04 requires downloading and installing a more recent
version of Boost.
...
Ubuntu 14.04) when compiling in C++11 mode (which Morphene does).
...
Then the instructions are the same as for morphene:
...
morphene. Until then, this will allow you to install boost 1.60.0.
...
### Clone the Repository
    ...
    git clone https://github.com/morphene/morphene.git
    cd morphene
...
e.g.:
    ...
    make -j$(sysctl -n hw.logicalcpu) morphened

This will only build `morphened`.
