 ...
 */
#pragma once
#include <morphene/chain/block_log.hpp>
#include <morphene/chain/fork_database.hpp>
#include <morphene/chain/global_property_object.hpp>
#include <morphene/chain/hardfork_property_object.hpp>
#include <morphene/chain/node_property_object.hpp>
#include <morphene/chain/notifications.hpp>

#include <morphene/chain/util/advanced_benchmark_dumper.hpp>
#include <morphene/chain/util/signal.hpp>
...
namespace morphene { namespace chain {
   ...
   using morphene::protocol::operation;
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = MORPHENE_INIT_SUPPLY;
         ...
         chain_id_type morphene_chain_id = MORPHENE_CHAIN_ID;
         ...
         */
         ...
         legacy_asset get_effective_vesting_shares( const account_object& account, asset_symbol_type vested_symbol )const;
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns MORPHENE_NULL_WITNESS
          */
         ...
         legacy_asset create_vesting( const account_object& to_account, legacy_asset morphene );

         void        adjust_balance( const account_object& a, const legacy_asset& delta );
         void        adjust_balance( const account_name_type& name, const legacy_asset& delta );
         void        adjust_supply( const legacy_asset& delta, bool adjust_vesting = false );
         void        update_owner_authority( const account_object& account, const authority& owner_authority );

         legacy_asset       get_balance( const account_object& a, asset_symbol_type symbol )const;
         legacy_asset       get_balance( const string& aname, asset_symbol_type symbol )const { return get_balance( get_account(aname), symbol ); }
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = MORPHENE_INIT_SUPPLY );
   ...
   protected:
      ...
      private:
         ...
         ///@}
         void modify_balance( const account_object& a, const legacy_asset& delta, bool check_balance );
         ...
         fc::time_point_sec            _hardfork_times[ MORPHENE_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ MORPHENE_NUM_HARDFORKS + 1 ];
