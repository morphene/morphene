...
#include <morphene/protocol/sign_state.hpp>

#include <morphene/chain/account_object.hpp>
#include <morphene/chain/database.hpp>

#include <morphene/plugins/auth_util/auth_util_api.hpp>
#include <morphene/plugins/auth_util/auth_util_plugin.hpp>

#include <fc/container/flat.hpp>

namespace morphene { namespace plugin { namespace auth_util {
...
{
   public:
      auth_util_api_impl( morphene::app::application& _app );
      void check_authority_signature( const check_authority_signature_params& args, check_authority_signature_result& result );

      std::shared_ptr< morphene::plugin::auth_util::auth_util_plugin > get_plugin();

      morphene::app::application& app;
};

auth_util_api_impl::auth_util_api_impl( morphene::app::application& _app ) : app( _app )
{}

std::shared_ptr< morphene::plugin::auth_util::auth_util_plugin > auth_util_api_impl::get_plugin()
{
...
auth_util_api::auth_util_api( const morphene::app::api_context& ctx )
{
