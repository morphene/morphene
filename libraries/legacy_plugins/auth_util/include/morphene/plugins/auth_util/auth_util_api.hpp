...
{
   public:
      auth_util_api( const morphene::app::api_context& ctx );
...
FC_REFLECT( morphene::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( morphene::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( morphene::plugin::auth_util::auth_util_api,
