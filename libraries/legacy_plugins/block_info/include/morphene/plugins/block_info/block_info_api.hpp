...
{
   public:
      block_info_api( const morphene::app::api_context& ctx );
...
FC_REFLECT( morphene::plugin::block_info::get_block_info_args,
   (start_block_num)
   (count)
   )

FC_API( morphene::plugin::block_info::block_info_api,
   (get_block_info)
