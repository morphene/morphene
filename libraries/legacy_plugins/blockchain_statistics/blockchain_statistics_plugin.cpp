#include <morphene/blockchain_statistics/blockchain_statistics_api.hpp>

#include <morphene/app/impacted.hpp>
...
#include <morphene/chain/operation_notification.hpp>
...
struct operation_process
{
   ...
   void operator()( const transfer_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.amount.symbol == MORPH_SYMBOL )
            b.morph_transferred += op.amount.amount;
      });
   }
   ...
   void operator()( const transfer_to_vesting_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         b.morph_vested += op.amount.amount;
      });
   }
   ...
   void operator()( const fill_vesting_withdraw_operation& op )const
   {
      ...
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.deposited.symbol == MORPH_SYMBOL )
...
void blockchain_statistics_plugin_impl::pre_operation( const operation_notification& o )
{
   ...
   for( const auto& bucket_id : _current_buckets )
   {
      if( o.op.which() == operation::tag< withdraw_vesting_operation >::value )
      {
         ...
         auto new_vesting_withdrawal_rate = op.vesting_shares.amount / MORPHENE_VESTING_WITHDRAW_INTERVALS;
...
MORPHENE_DEFINE_PLUGIN( blockchain_statistics, morphene::blockchain_statistics::blockchain_statistics_plugin );
