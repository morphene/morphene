...
#include <morphene/plugins/rc/rc_curve.hpp>
#include <morphene/plugins/rc/rc_export_objects.hpp>
#include <morphene/plugins/rc/rc_plugin.hpp>
#include <morphene/plugins/rc/rc_objects.hpp>
...
#include <morphene/jsonball/jsonball.hpp>

#define MORPHENE_RC_REGEN_TIME   (60*60*24*5)
#define MORPHENE_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT      293313625

#ifndef IS_TEST_NET
#define MORPHENE_HF20_BLOCK_NUM                              1
#endif

// 1.66% is ~2 hours of regen.
// 2 / ( 24 * 5 ) = 0.01666...
#define MORPHENE_RC_MAX_NEGATIVE_PERCENT 166

namespace morphene { namespace plugins { namespace rc {

using morphene::plugins::block_data_export::block_data_export_plugin;

namespace detail {

using chain::plugin_exception;
using morphene::chain::util::manabar_params;
...
void create_rc_account( database& db, uint32_t now, const account_object& account, legacy_asset max_rc_creation_adjustment )
{
   ...
   if( max_rc_creation_adjustment.symbol == MORPH_SYMBOL )
   {
   ...
   else
   {
      ...
      max_rc_creation_adjustment = legacy_asset( 0, VESTS_SYMBOL );
   }
...
template< bool account_may_exist = false >
void create_rc_account( database& db, uint32_t now, const account_name_type& account_name, legacy_asset max_rc_creation_adjustment )
{
   ...
   rc_plugin_skip_flags skip )
{
   if( account_name == account_name_type() )
   {
      if( db.is_producing() )
      {
         MORPHENE_ASSERT( false, plugin_exception,
   ...
   mbparams.regen_time = MORPHENE_RC_REGEN_TIME;
   ...
   db.modify( rc_account, [&]( rc_account_object& rca )
   {
      ...
      if( (!skip.skip_reject_not_enough_rc) )
      {
         if( db.is_producing() )
         {
            MORPHENE_ASSERT( has_mana, plugin_exception,
               "Account: ${account} has ${rc_current} RC, needs ${rc_needed} RC. Please wait to transact, or power up MORPH.",
      ...
      int64_t min_mana = -1 * ( MORPHENE_RC_MAX_NEGATIVE_PERCENT * mbparams.max_mana ) / MORPHENE_100_PERCENT;
...
void rc_plugin_impl::on_post_apply_transaction( const transaction_notification& note )
{
   ...
   int64_t rc_regen = (gpo.total_vesting_shares.amount.value / (MORPHENE_RC_REGEN_TIME / MORPHENE_BLOCK_INTERVAL));
   ...
   if( rc_regen > 0 )
   {
      for( size_t i=0; i<MORPHENE_NUM_RESOURCE_TYPES; i++ )
      {
   ...
   std::shared_ptr< exp_rc_data > export_data =
      morphene::plugins::block_data_export::find_export_data< exp_rc_data >( MORPHENE_RC_PLUGIN_NAME );
      ...
      [&]( rc_pool_object& pool_obj )
      {
         ...
         for( size_t i=0; i<MORPHENE_NUM_RESOURCE_TYPES; i++ )
         {
            ...
            if( debug_print )
            {
               ...
               a /= k*double(pool);
               dlog( "a=${a}   aR=${aR}", ("a", a)("aR", a*gpo.total_vesting_shares.amount.value/MORPHENE_RC_REGEN_TIME) );
            }
         }
         if( debug_print )
         {
...
void rc_plugin_impl::on_first_block()
{
   ...
   std::string resource_params_json = morphene::jsonball::get_resource_parameters();
   ...
   for( auto it=idx.begin(); it!=idx.end(); ++it )
   {
      create_rc_account( _db, now.sec_since_epoch(), *it, legacy_asset( MORPHENE_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT, VESTS_SYMBOL ) );
   }
...
struct pre_apply_operation_visitor
{
   ...
   void regenerate( const account_object& account, const rc_account_object& rc_account )const
   {
      ...
      // TODO:  Issue number
      //
      static_assert( MORPHENE_RC_REGEN_TIME <= MORPHENE_VOTING_MANA_REGENERATION_SECONDS, "RC regen time must be smaller than vote regen time" );
...
struct post_apply_operation_visitor
{
   ...
   void operator()( const clear_null_account_balance_operation& op )const
   {
      _mod_accounts.emplace_back( MORPHENE_NULL_ACCOUNT );
   }
...
void rc_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      if( export_plugin != nullptr )
      {
         ...
         export_plugin->register_export_data_factory( MORPHENE_RC_PLUGIN_NAME,
            []() -> std::shared_ptr< exportable_block_data > { return std::make_shared< exp_rc_data >(); } );
      }

      chain::database& db = appbase::app().get_plugin< morphene::plugins::chain::chain_plugin >().db();
      ...
      {
         my->_enable_at_block = MORPHENE_HF20_BLOCK_NUM;
      }
#endif
