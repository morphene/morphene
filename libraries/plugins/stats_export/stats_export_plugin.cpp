...
#include <morphene/plugins/stats_export/stats_export_plugin.hpp>
...
using morphene::chain::block_notification;
using morphene::chain::database;
using morphene::chain::dynamic_global_property_object;
...
using morphene::plugins::chain::chain_plugin;
...
FC_REFLECT( morphene::plugins::stats_export::detail::api_stats_transaction_data_object, (user)(size) )
FC_REFLECT( morphene::plugins::stats_export::detail::api_stats_export_data_object, (global_properties)(transaction_stats)(free_memory) )

namespace morphene { namespace plugins { namespace stats_export { namespace detail {
...
{
   std::shared_ptr< api_stats_export_data_object > stats = _export_plugin.find_export_data< api_stats_export_data_object >( MORPHENE_STATS_EXPORT_PLUGIN_NAME );
   if( !stats )
      return;
...
void stats_export_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      my->_export_plugin.register_export_data_factory( MORPHENE_STATS_EXPORT_PLUGIN_NAME,
