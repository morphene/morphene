...
using namespace morphene::utilities;
...
{
   public:
      wallet_api( const wallet_data& initial_data, const morphene::protocol::chain_id_type& _morphene_chain_id, fc::api< remote_node_api > rapi, fc::api< remote_node_api > ahapi );
      virtual ~wallet_api();
       ...
       */
      database_api::state get_state( string url );
       ...
       *
       * @param permlink the permlink of the auction to query
       * @param limit the maximum number of auctions to return
       * @returns the auction object stored in the blockchain
       */
      vector< database_api::api_bid_object > get_bids( string permlink, uint32_t limit ) const;
       ...
       */
      database_api::signed_transaction get_transaction( transaction_id_type trx_id )const;
       ...
       */
      database_api::signed_transaction create_account( string creator, string new_account_name, string json_meta, bool broadcast );
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction create_account_with_keys(
         string creator,
         string newname,
         string json_meta,
       ...
       *  'info' wallet command.
       *
       *  These accounts are created with combination of MORPH and delegated SP
       *
       *  @param creator The account creating the new account
       *  @param morph_fee The amount of the fee to be paid with MORPH
       *  @param delegated_vests The amount of the fee to be paid with delegation
       *  @param new_account_name The name of the new account
       *  @param json_meta JSON Metadata associated with the new account
       *  @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction create_account_delegated(
         string creator,
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction create_account_with_keys_delegated(
         string creator,
         legacy_asset morph_fee,
         legacy_asset delegated_vests,
         string newname,
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction update_account(
         string accountname,
         string json_meta,
         public_key_type owner,
         public_key_type active,
       ...
       * @param broadcast true if you wish to broadcast the transaction.
       */
      database_api::signed_transaction update_account_auth_key(
         string account_name,
         authority_type type,
         public_key_type key,
         weight_type weight,
       ...
       * @param broadcast true if you wish to broadcast the transaction.
       */
      database_api::signed_transaction update_account_auth_account(
         string account_name,
         authority_type type,
         string auth_account,
         weight_type weight,
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction update_account_auth_threshold(
         string account_name,
         authority_type type,
         uint32_t threshold,
         bool broadcast );
       ...
       */
      database_api::signed_transaction update_account_meta(
         string account_name,
         string json_meta,
         bool broadcast );
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction update_account_memo_key(
         string account_name,
         public_key_type key,
         bool broadcast );
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
       database_api::signed_transaction delegate_vesting_shares(
          string delegator,
          string delegatee,
       ...
       * @param broadcast true if you wish to broadcast the transaction.
       */
      database_api::signed_transaction update_witness(
         string witness_name,
         string url,
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction set_voting_proxy(
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction vote_for_witness(
      ...
      /**
       * Transfer funds from one account to another. MORPH can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 MORPH"
       * @param memo A memo for the transactionm, encrypted with the to account's public memo key
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction transfer(
         string from,
         string to,
         legacy_asset amount,
         string memo,
       ...
       *
       * @param from The account the MORPH is coming from
       * @param to The account getting the VESTS
       * @param amount The amount of MORPH to vest i.e. "100.00 MORPH"
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction transfer_to_vesting(
         string from,
         string to,
         legacy_asset amount,
         bool broadcast = false);
       ...
       *    withdrawn and deposited back as MORPH. i.e. "10.000000 VESTS"
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction withdraw_vesting(
         string from,
         legacy_asset vesting_shares,
         bool broadcast = false );
       ...
       *
       * @param from The account the VESTS are withdrawn from.
       * @param to   The account receiving either VESTS or MORPH.
       ...
       * @param broadcast true if you wish to broadcast the transaction.
       */
      database_api::signed_transaction set_withdraw_vesting_route(
         string from,
         string to,
         uint16_t percent,
         bool auto_vest,
       ...
       */
      database_api::signed_transaction sign_transaction(
         database_api::signed_transaction tx,
         bool broadcast = false);
       ...
       *                       operations defined in `morphene/chain/operations.hpp`
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction request_account_recovery(
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction recover_account(
         string account_to_recover,
       ...
       * @param broadcast true if you wish to broadcast the transaction
       */
      database_api::signed_transaction change_recovery_account(
         string owner,
       ...
       */
      vector< account_history::api_operation_object > get_account_history( account_name_type account, uint64_t from, uint32_t limit );
...
FC_REFLECT( morphene::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( morphene::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( morphene::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( morphene::wallet::authority_type, (owner)(active)(posting) )

FC_API( morphene::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
