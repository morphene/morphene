...
#include <morphene/utilities/words.hpp>

#include <morphene/protocol/base.hpp>
#include <morphene/wallet/wallet.hpp>
#include <morphene/wallet/api_documentation.hpp>
#include <morphene/wallet/reflect_util.hpp>
...
public:
   wallet_api& self;
   wallet_api_impl( wallet_api& s, const wallet_data& initial_data, const morphene::protocol::chain_id_type& _morphene_chain_id, fc::api< remote_node_api > rapi, fc::api< remote_node_api > ahapi )
      : self( s ),
        _remote_api( rapi ),
        _account_history_api( ahapi )
   {
      init_prototype_ops();

      _wallet.ws_server = initial_data.ws_server;
      morphene_chain_id = _morphene_chain_id;
   }
   ...
   {
      string client_version( morphene::utilities::git_revision_description );
      const size_t pos = client_version.find( '/' );
      if( pos != string::npos && client_version.size() > pos )
         client_version = client_version.substr( pos + 1 );

      fc::mutable_variant_object result;
      result["blockchain_version"]       = MORPHENE_BLOCKCHAIN_VERSION;
      result["client_version"]           = client_version;
      result["morphene_revision"]           = morphene::utilities::git_revision_sha;
      result["morphene_revision_age"]       = fc::get_approximate_relative_time_string( fc::time_point_sec( morphene::utilities::git_revision_unix_timestamp ) );
      ...
      try
      {
         ...
         result["server_morphene_revision"] = v.morphene_revision;
   ...
   bool import_key(string wif_key)
   {
      ...
      morphene::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         morphene::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         morphene::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         morphene::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         morphene::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
         ...
         if( broadcast )
         {
            auto result = _remote_api->broadcast_transaction_synchronous( database_api::signed_transaction( tx ) );
            FC_UNUSED(result);
         }
         return tx;
   ...
   {
      FC_ASSERT( tx_expiration_seconds < MORPHENE_MAX_TIME_UNTIL_EXPIRATION );
      ...
      bool broadcast = false )
   {
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, morphene_chain_id, fc::ecc::fc_canonical );
      }
   ...
   wallet_data                             _wallet;
   morphene::protocol::chain_id_type          morphene_chain_id;

   map<public_key_type,string>             _keys;
   fc::sha512                              _checksum;
   fc::api< remote_node_api >              _remote_api;
   fc::api< remote_node_api >              _account_history_api;
...
} } } // morphene::wallet::detail



namespace morphene { namespace wallet {

wallet_api::wallet_api(const wallet_data& initial_data, const morphene::protocol::chain_id_type& _morphene_chain_id, fc::api< remote_node_api > rapi, fc::api< remote_node_api > ahapi)
   : my(new detail::wallet_api_impl(*this, initial_data, _morphene_chain_id, rapi, ahapi))
{}
...
brain_key_info wallet_api::suggest_brain_key()const
{
   ...
   {
      fc::bigint choice = entropy % morphene::words::word_list_size;
      entropy /= morphene::words::word_list_size;
      if( i > 0 )
         brain_key += " ";
      brain_key += morphene::words::word_list[ choice.to_int64() ];
   }
...
database_api::signed_transaction wallet_api::set_voting_proxy(string account_to_modify, string voting_account, bool broadcast /* = false */)
{ return my->set_voting_proxy(account_to_modify, voting_account, broadcast); }

void wallet_api::set_wallet_filename(string wallet_filename) { my->_wallet_filename = wallet_filename; }

database_api::signed_transaction wallet_api::sign_transaction(
   database_api::signed_transaction tx, bool broadcast /* = false */)
{ try {
   signed_transaction appbase_tx( tx );
   database_api::annotated_signed_transaction result = my->sign_transaction( appbase_tx, broadcast);
   return database_api::signed_transaction( result );
 ...
 * wallet.
 */
database_api::signed_transaction wallet_api::create_account_with_keys(
   string creator,
...
database_api::signed_transaction wallet_api::recover_account( string account_to_recover, authority recent_authority, authority new_authority, bool broadcast ) {
   FC_ASSERT( !is_locked() );
...
database_api::signed_transaction wallet_api::change_recovery_account( string owner, string new_recovery_account, bool broadcast ) {
   FC_ASSERT( !is_locked() );
...
vector< account_history::api_operation_object > wallet_api::get_account_history( account_name_type account, uint64_t from, uint32_t limit )
{
   vector< account_history::api_operation_object > result = my->_remote_api->get_account_history(account, from, limit);
   if( !is_locked() )
   {
...
database_api::signed_transaction wallet_api::update_account(
   string account_name,
   string json_meta,
   public_key_type owner,
   public_key_type active,
...
database_api::signed_transaction wallet_api::update_account_auth_key(
   string account_name,
   authority_type type,
   public_key_type key,
   weight_type weight,
...
database_api::signed_transaction wallet_api::update_account_auth_account(
   string account_name,
   authority_type type,
   string auth_account,
   weight_type weight,
...
database_api::signed_transaction wallet_api::update_account_auth_threshold(
   string account_name,
   authority_type type,
   uint32_t threshold,
   bool broadcast )
...
database_api::signed_transaction wallet_api::update_account_meta(
   string account_name,
   string json_meta,
   bool broadcast )
{
...
database_api::signed_transaction wallet_api::update_account_memo_key(
   string account_name,
   public_key_type key,
   bool broadcast )
{
...
database_api::signed_transaction wallet_api::delegate_vesting_shares(
   string delegator,
   string delegatee,
 ...
 */
database_api::signed_transaction wallet_api::create_account(
   string creator,
   string new_account_name,
   string json_meta,
   bool broadcast )
...
database_api::signed_transaction wallet_api::update_witness(
   ...
   const database_api::legacy_chain_properties& props,
   bool broadcast  )
{
   FC_ASSERT( !is_locked() );
...
database_api::signed_transaction wallet_api::vote_for_witness(
...
database_api::signed_transaction wallet_api::transfer(
   string from,
   string to,
   legacy_asset amount,
   string memo,
...
database_api::signed_transaction wallet_api::escrow_transfer(
   string from,
   string to,
   string agent,
   uint32_t escrow_id,
...
database_api::signed_transaction wallet_api::escrow_approve(
   string from,
   string to,
   string agent,
   string who,
...
database_api::signed_transaction wallet_api::escrow_dispute(
   string from,
   string to,
   string agent,
   string who,
...
database_api::signed_transaction wallet_api::escrow_release(
   string from,
   string to,
   string agent,
   string who,
...
database_api::signed_transaction wallet_api::transfer_to_vesting(
   string from,
   string to,
   legacy_asset amount,
   bool broadcast )
...
database_api::signed_transaction wallet_api::withdraw_vesting(
   string from,
   legacy_asset vesting_shares,
   bool broadcast )
{
...
database_api::signed_transaction wallet_api::set_withdraw_vesting_route(
   string from,
   string to,
   uint16_t percent,
   bool auto_vest,
...
database_api::state wallet_api::get_state( string url )
{
