...
{
   morphene::protocol::transaction     tx;
   std::string                      wif;
};
...
struct tx_signing_result
{
   ...
   morphene::protocol::public_key_type key;
   morphene::protocol::signature_type  sig;
};
...
int main(int argc, char** argv, char** envp)
{
   ...
   while( std::cin )
   {
      ...
      try
      {
         ...
         auto priv_key = morphene::utilities::wif_to_key( sreq.wif );

         if(priv_key)
         {
            sres.sig = priv_key->sign_compact( sres.sig_digest );
            sres.key = morphene::protocol::public_key_type( priv_key->get_public_key() );
